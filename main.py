...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    await message.reply("Название группы")
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    await message.reply("Год поступления", reply_markup=markup)
@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020"], state=Form.startedu)
async def process_gender_invalid(message: types.Message):
    return await message.reply("Ответ не верный. Попробуйте еще раз.")
...
async def process_gender(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(
            message.chat.id,
            md.text(
                md.text('Здравствуйте,', md.bold(data['firstname'] + " " + data['lastname'])),
                md.text('Группа:', data['group']),
                md.text('Начало обучения :', data['startedu']),
                sep='\n',
            ),
            reply_markup=markup,
